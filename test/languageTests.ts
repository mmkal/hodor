...
const validScriptTests: { [name: string]: string } = {
  ...
  booleanOperators:
    "$val = (1 < 7) Ho-dor (1 > 8) Hod-or (1 <= 9) != (1 >= 10) == (1 <= 11);",
...
Object.keys(validScriptTests).forEach((name) => {
  test(name + " script doesn't throw", () => {
...
const invalidScriptTests: { [name: string]: string } = {
  ...
  "missing then:Expecting keyword":
    "Hodor? (1 < 2) $print(HODOR) Hodor!! $print(hodor);",
  "undefined variable in scope:Undefined variable": `$f = Hodor($x) { $print($y); }; $f(1);`,
  "weird tilde:Unexpected character: ~": "~",
  'unterminated strinng literal: Expected to find ""@" but reached end of file':
    '$x = @"hello',
};
...
Object.keys(invalidScriptTests).forEach((info) => {
  ...
  test(name + " throws", () => {
    ...
    expect(() => executeAndGetOutput(code)).toThrowError(
      new RegExp(errorRegex)
    );
  });
});
